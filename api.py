...
def _load_secret(var_name: str, config_key: Optional[str] = None, config_path: str = "config.json") -> Optional[str]:
    ...
    if config_key and os.path.exists(config_path):
        try:
...
DISCORD_TOKEN = _load_secret("DISCORD_TOKEN", "token")
GTC_API_KEY = _load_secret("GTC_API_KEY", "GTC_API_KEY")
PANEL_CHANNEL_ID = int(_load_secret("PANEL_CHANNEL_ID", "auth_channel_id") or 1503503368176402606)
...
if __name__ == "__main__":
    if not DISCORD_TOKEN:
        raise RuntimeError(
            "DISCORD_TOKEN no encontrado. Define la variable de entorno DISCORD_TOKEN o crea un archivo .env con DISCORD_TOKEN=<tu_token>"
        )
    if not GTC_API_KEY:
        raise RuntimeError(
            "GTC_API_KEY no encontrado. Define la variable de entorno GTC_API_KEY o crea un archivo .env con GTC_API_KEY=<tu_api_key>"
        )
