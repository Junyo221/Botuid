...
if load_dotenv:
    ...
    logger.info("python-dotenv no instalado: cargando .env manualmente si existe")
...
def _get_env_var(name: str, required: bool = False) -> Optional[str]:
    value = os.getenv(name)
    if value:
        return value.strip()
    if required:
        raise RuntimeError(
            f"{name} no encontrado. Define la variable de entorno {name} "
            "o crea un archivo .env con esa clave."
        )
    return None


DISCORD_TOKEN = _get_env_var("DISCORD_TOKEN", required=True)
GTC_API_KEY = _get_env_var("GTC_API_KEY", required=True)
PANEL_CHANNEL_ID = int(_get_env_var("PANEL_CHANNEL_ID") or "1503503368176402606")
